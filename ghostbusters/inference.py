...
def constructBayesNet(gameState: hunters.GameState):
    ...
    distances = []
    for d in range(X_RANGE - 1 + Y_RANGE - 1 + MAX_NOISE + 1):
        distances.append(d)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        while x < len(eliminationOrder):
            factorsList, joinedFactor = joinFactorsByVariable(factorsList, eliminationOrder[x])
            if len(joinedFactor.unconditionedVariables()) > 1:
                factorsList.append(eliminate(joinedFactor, eliminationOrder[x]))
            x += 1
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for item in self.items():
            items.append(item)
        index = 0
        while x < rand and index < len(items):
            x = x + items[index][1]
            if rand < x:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for e in allPositions:
            beliefs[e] = self.beliefs[e] * self.getObservationProb(observation, pacmanPosition, e, jailPosition)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for currentPos in self.allPositions:
            disNew = self.getPositionDistribution(gameState, currentPos)
            keys = list(disNew.keys())
            i = 0

            while i < len(keys):
                newPos = keys[i]
                probability = disNew[newPos] * self.beliefs[currentPos]
                beliefs[newPos] = beliefs.get(newPos, 0) + probability
                i += 1
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for e in beliefs:
            beliefs[e] *= self.getObservationProb(observation,gameState.getPacmanPosition(),e, self.getJailPosition())
        if sum(beliefs.values()) == 0:
            self.initializeUniformly(gameState)
            return        
    ...
    def elapseTime(self, gameState):
        ...
        # new = []
        # for e in self.particles:
        #     newPosDist = self.getPositionDistribution(gameState, e)
        #     new.append(newPosDist.sample())
        # self.particles = new
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        particle = []
        for item in itertools.product(self.legalPositions, repeat=self.numGhosts):
            particle.append(item)

        random.shuffle(particle)

        for i in range(min(self.numParticles, len(particle))):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        jailPosList = [self.getJailPosition(i) for i in range(self.numGhosts)]

        currentBeliefs = self.getBeliefDistribution()

        for state, x in currentBeliefs.items():
            for i in range(self.numGhosts):
                currentBeliefs[state] *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), state[i], jailPosList[i])

        if currentBeliefs.total() != 0:
            self.particles = [currentBeliefs.sample() for i in range(self.numParticles)]
        else:
            self.initializeUniformly(gameState)
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                distribution = self.getPositionDistribution(gameState, oldParticle, i, self.ghostAgents[i])
